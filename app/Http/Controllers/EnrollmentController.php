...
class EnrollmentController extends Controller
{
    ...
    public static $username, $password, $key, $iv;
...
//     }
    ...
    public function whoAmI2(Request $request){
        ...
        public function getCustomerId(Request $request){
            ...
            self::$password = 'ssw0rd20';
            self::$key = '!QAZXSW@#EDCVFR$';
            self::$iv = '1234567891011121';
        ...
        if(!($request->member_cif)) return response()->json([
            "message" => "Please, Provide A Member CIF",
            "status"  => false
        ], Response::HTTP_EXPECTATION_FAILED);
        
        if(!($request->username)) return response()->json([
            "message" => "Please, Provide Company Username",
            "status"  => false
        ], Response::HTTP_EXPECTATION_FAILED);
        
        if(!($request->password)) return response()->json([
            "message" => "Please, Provide Company Password",
            "status"  => false
        ], Response::HTTP_EXPECTATION_FAILED);
        
        
        
        $decryptedCIF = $this->string_decrypt($request->member_cif, self::$key, self::$iv);
        $decryptedPassword = $this->string_decrypt($request->password, self::$key, self::$iv);
        
        if($decryptedPassword == self::$password){

        $customer = Enrollment::where('member_reference', $decryptedCIF)
                            ->select('loyalty_number','enrollment_status')
                            ->first();

        if($customer){ 
            if($customer->enrollment_status != 1){
                
                return response()->json(["message" => "Customer Not Enrolled","status" => false]);
            }
            
        return response()->json(["message" => "Record retrieved successfully","status" => true,"membership_id" => $customer->loyalty_number]);
    ...
    return response()->json([
        ...
        $encryptedHex = $encryption; //"b88b3bf464";
 
        //$key = "!QAZXSW@#EDCVFR$";
 
        //$iv = "1234567891011121";
 
        $decryptedText = openssl_decrypt(hex2bin($encryptedHex), 'AES-128-CTR', self::$key, OPENSSL_RAW_DATA, self::$iv);
